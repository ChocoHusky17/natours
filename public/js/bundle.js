...
var displayMap = function displayMap(locations) {
  var lng = locations[0].coordinates[0];
  var lat = locations[0].coordinates[1];
  var map = L.map('map').setView([lat, lng], 13); // L : init in leaflet.js
  ...
  googleTerrain.addTo(map); // ADD CUSTOM ICON/MARKER
  ...
  var markers = [];
  var label; // loop add marker and label

  locations.forEach(function (location, i) {
    lng = location.coordinates[0];
    lat = location.coordinates[1];
    label = "<div style='font-size:13px'><b>Day ".concat(location.day, " : ").concat(location.description.toUpperCase(), "</></div>");
      ...
      while (1) {
        switch (_context.prev = _context.next) {
          ...
          case 4:
            ...
            if (res.data.status === 'success') {
              (0, _alert.showAlert)('success', "Your ".concat(type.toUpperCase(), " updated successfully!"));
            }
    ...
    return _regeneratorRuntime().wrap(function _callee$(_context) {
      while (1) {
        switch (_context.prev = _context.next) {
          case 0:
            _context.prev = 0;
            stripe = Stripe('pk_test_51LNZckL3jckCLx9IkZ7xFrUY6DGq3Sp0JDJz8JAoOXD9GrNCpVKdc53DvjGdukeDCWygUNfiMRUqdAxcSUyHOb4y0019o75GC1'); // 1) Get checkout session from API
...
if (settingForm) {
  settingForm.addEventListener('submit', /*#__PURE__*/function () {
      ...
      return _regeneratorRuntime().wrap(function _callee$(_context) {
        while (1) {
          switch (_context.prev = _context.next) {
            case 0:
              e.preventDefault();
              document.querySelector('.btn--save--data').textContent = 'Updating...'; // VALUES
